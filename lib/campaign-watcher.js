...
class CampaignWatcher {
  ...
  isWithinSchedule() {
    ...
    const now = new Date();
    const agentTime = new Date(now.toLocaleString('en-US', { timeZone: this.agentTimezone }));
    
    // Get current day of week (0 = Sunday, 1 = Monday, etc.)
    const dayOfWeek = agentTime.getDay();
    const currentHour = agentTime.getHours();
    const currentMinute = agentTime.getMinutes();
    ...
    const todaySchedule = this.agentSchedule[currentDayName] || [];
    ...
    const nextWindow = this.findNextAvailableWindow(agentTime);
  ...
  findNextAvailableWindow(currentTime) {
    ...
    const todaySchedule = this.agentSchedule[dayNames[currentDayIndex]] || [];
    for (const range of todaySchedule) {
      const startMinutes = range.start.hour * 60 + range.start.minute;
      if (startMinutes > currentTimeMinutes) {
        const nextTime = new Date(currentTime);
        nextTime.setHours(range.start.hour, range.start.minute, 0, 0);
        return this.formatScheduleTime(nextTime, range);
      }
    }
  ...
  formatScheduleDisplay() {
    ...
    for (const day of dayNames) {
      const daySchedule = this.agentSchedule[day] || [];
